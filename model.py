...
class ValueNetwork(nn.Module):
    def __init__(self, num_inputs, hidden_sizes,activation):
        ...
        self.apply(weights_init_(seed=1)) #self.apply() 方法是 nn.Module 类中的一个方法，它会递归地遍历模型的所有子模块，并对每个子模块应用一个函数。
...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_sizes,activation):
        ...
        weights_init_(self.Q_network_1,seed=10)
        weights_init_(self.Q_network_2,seed=20)
...
class GaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_sizes,activation, action_space=None,RE_PARAMETERIZATION=True):
        ...
        weights_init_(self.pi_network,seed=30)
        weights_init_(self.mu_layer,seed=40)
        weights_init_(self.log_std_layer,seed=50)
